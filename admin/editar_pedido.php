...
$stmt = $db->prepare("SELECT * FROM pedido_itens WHERE pedido_id = ?");
...
<head>
    ...
    <title>Editar Pedido - Painel Administrativo</title>
...
<body class="bg-gray-50">
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Editar Pedido #<?php echo $pedido['id']; ?></h1>
            ...
            <form id="formPedido" class="space-y-6">
                ...
                <div>
                    ...
                    <select id="usuario_id" name="usuario_id" required
                        ...
                        <?php foreach ($usuarios as $usuario): ?>
                            <option value="<?php echo $usuario['id']; ?>" <?php echo $usuario['id'] == $pedido['usuario_id'] ? 'selected' : ''; ?>>
                    ...
                    <div id="lista_produtos" class="space-y-4">
                        ...
                        <?php foreach ($itens_pedido as $item): ?>
                            <div class="produto-item bg-gray-50 p-4 rounded-lg flex flex-wrap items-start gap-4">
                                        ...
                                        <optgroup label="<?php echo htmlspecialchars($categoria); ?>">
                                            <?php foreach ($produtos_categoria as $produto): ?>
                                                <option value="<?php echo $produto['id']; ?>" 
                                                        data-preco="<?php echo $produto['preco']; ?>"
                                                        data-tem-complementos="<?php echo $produto['tem_complementos']; ?>"
                                                        <?php echo $produto['id'] == $item['produto_id'] ? 'selected' : ''; ?>>
                                                    <?php echo htmlspecialchars($produto['nome']); ?> - 
                                                    R$ <?php echo number_format($produto['preco'], 2, ',', '.'); ?>
                                                </option>
                                            <?php endforeach; ?>
                                ...
                                <input type="number" name="produtos[][quantidade]" value="<?php echo $item['quantidade']; ?>" min="1" required
                                       class="w-24 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500"
                                       placeholder="Qtd">
                ...
                <div>
                    ...
                    <textarea id="observacao" name="observacao" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500"
                              placeholder="Alguma observação especial para este pedido?"><?php echo htmlspecialchars($pedido['observacao']); ?></textarea>
                </div>
