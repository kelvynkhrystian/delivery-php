...
foreach ($config as $key => $value) {
    if (empty($value) && !in_array($key, ['pedido_minimo', 'tempo_entrega', 'latitude_loja', 'longitude_loja', 'logo', 'banner', 'banner_pc', 'instagram', 'favicon', 'endereco_loja'])) {
...
<head>
    ...
    <script>
        ...
        function carregarConfiguracoesIniciais() {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    document.getElementById('maps_raio').value = data.raio_entrega || '';
        ...
        function atualizarCirculo() {
            const raio = parseFloat(document.getElementById('maps_raio').value) * 1000; // Converter km para metros
            if (raio > 0 && marker) {
        ...
        function initMap() {
            mapsLoaded = true;
            const lat = parseFloat(document.getElementById('maps_latitude').value) || -14.2350;
            const lng = parseFloat(document.getElementById('maps_longitude').value) || -51.9253;
            const location = { lat, lng };
            
            map = new google.maps.Map(document.getElementById('map'), {
                zoom: lat === -14.2350 ? 4 : 16,
                center: location,
            });
            ...
            // Criar círculo para raio de entrega
            ...
            marker.addListener('dragend', function() {
                ...
                document.getElementById('maps_latitude').value = position.lat();
                document.getElementById('maps_longitude').value = position.lng();
                atualizarCirculo();
                // Salva as configurações após mover o marcador
                salvarConfiguracoesMaps();
            });

            // Atualiza o círculo inicial
            atualizarCirculo();
        }
        ...
        function salvarConfiguracoesMaps() {
            const data = {
                ...
                raio_entrega: document.getElementById('maps_raio').value
            };
        ...
        function editarFaixaKm(id, button) {
            ...
            if (!isEditing) {
                ...
                // Salva alterações
                const inicio = parseFloat(inputs[0].value);
                const fim = parseFloat(inputs[1].value);
        ...
        function selecionarTema(tema) {
            ...
            .then(data => {
                if (data.success) {
                    // Busca a cor atual do CSS e do banco
                    const corAtualCSS = getComputedStyle(document.documentElement).getPropertyValue('--theme-color').trim();
                    
                    console.log('------- Alteração do Tema -------');
                    console.log('Tema selecionado:', tema);
                    console.log('Nova cor:', cor);
                    console.log('Cor atual no CSS:', corAtualCSS);
                    
                    // Busca os dados atualizados do banco
                    fetch('obter_tema_atual.php')
                    .then(response => response.json())
                    .then(temaAtual => {
                        console.log('Cor salva no banco:', temaAtual.cor_tema);
                        console.log('---------------------------');
        ...
        function salvarEdicao() {
            ...
            if (!formBairro.classList.contains('hidden')) {
                ...
                const nomeAntigo = document.getElementById('editBairroNomeAntigo').value;
                ...
                .then(data => {
                    if (data.success) {
                        // Limpa os campos
                        document.getElementById('editBairroNome').value = '';
                        document.getElementById('editBairroValor').value = '';
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Faixa de distância atualizada com sucesso!',
                            showConfirmButton: false,
                            timer: 1500
                        });
                        carregarFaixasKm();
                ...
                .catch(error => {
                    Swal.fire({
                        icon: 'error',
                        title: 'Erro ao atualizar faixa de distância',
                        text: error.message
                    });
                });
            }
...
<!-- Navbar -->
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div id="secao-entrega" class="secao hidden">
                <div class="bg-white rounded-lg shadow-sm p-4">
                    ...
                    <div id="secao-entrega-bairro" class="<?php echo $config['tipo_entrega'] === 'distancia' ? 'hidden' : ''; ?>">
                    ...
                    <div id="secao-entrega-distancia" class="<?php echo $config['tipo_entrega'] === 'bairro' ? 'hidden' : ''; ?>">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-lg font-medium">Configuração por Distância</h3>
                            <button onclick="abrirModalAdicao('distancia')" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-purple-600 rounded-md hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500">
                                <i class="fas fa-plus mr-2"></i>
                                Adicionar Faixa
                            </button>
                        </div>
                        ...
                        <!-- Lista de distâncias -->
                        <div id="lista_distancias" class="space-y-2">
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-4 gap-4 mb-6 grid-cores">
                            ...
                            <div class="flex flex-col items-center">
                                <button data-cor="#EAB308" data-nome="amarelo" 
                            ...
                            <!-- Laranja -->
                            <div class="flex flex-col items-center">
                                <button data-cor="#F97316" data-nome="laranja" 
                            ...
                            <!-- Vermelho -->
                            <div class="flex flex-col items-center">
                                <button data-cor="#EF4444" data-nome="vermelho" 
                            ...
                            <!-- Verde -->
                            <div class="flex flex-col items-center">
                                <button data-cor="#22C55E" data-nome="verde" 
                            ...
                            <!-- Azul -->
                            <div class="flex flex-col items-center">
                                <button data-cor="#3B82F6" data-nome="azul" 
                            ...
                            <!-- Roxo -->
                            <div class="flex flex-col items-center">
                                <button data-cor="#8B5CF6" data-nome="roxo" 
                                    class="w-16 h-16 rounded-xl hover:scale-105 transition-transform shadow-md hover:shadow-lg active:scale-95<?php echo $config['cor_tema'] === '#8B5CF6' ? ' ring-2 ring-offset-2 ring-purple-500' : ''; ?>" 
                            ...
                            <!-- Preto -->
                            <div class="flex flex-col items-center">
                                <button data-cor="#111827" data-nome="preto" 
                                    class="w-16 h-16 rounded-xl bg-gray-900 hover:scale-105 transition-transform shadow-md hover:shadow-lg active:scale-95<?php echo $config['cor_tema'] === '#111827' ? ' ring-2 ring-offset-2 ring-gray-900' : ''; ?>" 
                                    title="Preto"></button>
                                <span class="text-sm mt-2">Preto</span>
                            </div>
                            ...
                            <div class="flex flex-col items-center">
                                <div class="relative">
                                    ...
                                    <input type="color" id="cor_personalizada" value="<?php echo $config['cor_tema'] ?? '#8B5CF6'; ?>" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" title="Escolha uma cor">
                                </div>
            ...
            <div id="secao-maps" class="secao hidden">
                <div class="bg-white rounded-lg shadow-sm p-4">
                        ...
                        <div class="flex flex-col md:flex-row gap-2">
                            <div class="flex flex-1 gap-2 my-2 md:my-0">
                                <input type="password" id="maps_api_key" 
                                    class="flex-1 h-12 md:h-10 rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500"
                                    placeholder="Insira sua API Key do Google Maps">
                                <button onclick="toggleApiKeyVisibility()" class="bg-gray-100 px-3 rounded-md">
                                    <i class="fas fa-eye"></i>
                                </button>
                            </div>
                            <button onclick="validarApiKey()" class="bg-purple-500 text-white px-4 py-2 rounded-md hover:bg-purple-600">
                                <i class="fas fa-check"></i> Validar e Salvar
                            </button>
                        </div>
                        <p class="text-sm text-gray-500 mt-2">
                            <i class="fas fa-info-circle"></i> 
                            Obtenha sua API Key no <a href="https://console.cloud.google.com" target="_blank" class="text-purple-500 hover:text-purple-700">Google Cloud Console</a>
                        </p>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <!-- Mapa -->
                        <div id="map" class="w-full h-96 rounded-lg border border-gray-300 mb-4"></div>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input type="text" id="maps_latitude" readonly
                                    class="w-full h-10 rounded-md border-gray-300 bg-gray-50">
                            </div>
                            <div>
                        ...
                        <!-- Raio de Entrega -->
                        <div class="mb-4">
                            <label class="block text-gray-700 text-sm font-bold mb-2" for="maps_raio">
                                Raio de Entrega (km)
                            </label>
                            <input type="number" id="maps_raio" min="0" step="1"
                                class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
    ...
    <div id="modalEdicao" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden">
        <div class="bg-white rounded-lg p-6 w-96">
            ...
            <!-- Formulário de Edição de Bairro -->
            <form id="formBairro" class="space-y-4 hidden">
                <input type="hidden" id="editBairroNomeAntigo">
                <div>
                    <label for="editBairroNome" class="block text-sm font-medium text-gray-700">Nome do Bairro</label>
                    <input type="text" id="editBairroNome" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                </div>
                <div>
                    <label for="editBairroValor" class="block text-sm font-medium text-gray-700">Valor da Entrega</label>
                    <div class="mt-1 relative rounded-md shadow-sm">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <span class="text-gray-500 sm:text-sm">R$</span>
                        </div>
                        <input type="number" step="0.01" min="0" id="editBairroValor" class="pl-10 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                    </div>
                </div>
            </form>
            
            <!-- Formulário de Edição de Distância -->
            <form id="formDistancia" class="space-y-4 hidden">
                <input type="hidden" id="editDistanciaId">
                <div>
                    <label for="editDistanciaDe" class="block text-sm font-medium text-gray-700">De (km)</label>
                    <input type="number" id="editDistanciaDe" step="0.001" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                </div>
                <div>
                    <label for="editDistanciaAte" class="block text-sm font-medium text-gray-700">Até (km)</label>
                    <input type="number" id="editDistanciaAte" step="0.001" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                </div>
                <div>
                    <label for="editDistanciaValor" class="block text-sm font-medium text-gray-700">Valor da Entrega</label>
                    <div class="mt-1 relative rounded-md shadow-sm">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <span class="text-gray-500 sm:text-sm">R$</span>
                        </div>
                        <input type="number" step="0.01" min="0" id="editDistanciaValor" class="pl-10 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                    </div>
                </div>
            </form>
            
            <div class="mt-6 flex justify-end gap-3">
                <button onclick="fecharModal()" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-md hover:bg-gray-200">
                    Cancelar
                </button>
                <button onclick="salvarEdicao()" class="px-4 py-2 bg-purple-500 text-white rounded-md hover:bg-purple-600">
                    Salvar
                </button>
            </div>
        </div>
    ...
    <div id="modalAdicao" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden">
        <div class="bg-white rounded-lg p-6 w-96">
            <div class="flex justify-between items-center mb-4">
                <h3 id="modalAdicaoTitle" class="text-lg font-medium"></h3>
            ...
            <div id="formAdicaoBairro" class="hidden">
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" id="addBairroNome" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Valor da Entrega</label>
                        <input type="number" id="addBairroValor" step="0.01" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                    </div>
                </div>
            </div>
            ...
            <div id="formAdicaoDistancia" class="hidden">
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">De (km)</label>
                        <input type="number" id="addDistanciaDe" step="0.001" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Até (km)</label>
                        <input type="number" id="addDistanciaAte" step="0.001" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Valor da Entrega</label>
                        <input type="number" id="addDistanciaValor" step="0.01" min="0" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring-purple-500">
                    </div>
                </div>
            </div>

            <div class="mt-6 flex justify-end gap-3">
                <button onclick="fecharModalAdicao()" class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">
                    Cancelar
                </button>
                <button onclick="salvarAdicao()" class="px-4 py-2 bg-purple-500 text-white rounded-md hover:bg-purple-600">
                    Adicionar
                </button>
            </div>
        </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function atualizarTema(cor) {
                ...
                document.querySelectorAll('.ring-primary').forEach(el => {
                    el.style.setProperty('--tw-ring-color', cor);
                });

                // Atualiza o color picker
                document.getElementById('cor_personalizada').value = cor;

                // Remove o anel de seleção de todos os botões
                document.querySelectorAll('.grid-cores button').forEach(btn => {
                    btn.classList.remove('ring-2', 'ring-offset-2');
                });

                // Adiciona o anel no botão da cor selecionada
                const botaoSelecionado = document.querySelector(`[data-cor="${cor}"]`);
                if (botaoSelecionado) {
                    const ringClass = `ring-${botaoSelecionado.dataset.nome}-500`;
                    botaoSelecionado.classList.add('ring-2', 'ring-offset-2', ringClass);
                }
            }

            // Adiciona evento de clique para todos os botões de cor
            document.querySelectorAll('.grid-cores button').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    // Faz a requisição para salvar a cor
                    fetch('salvar_configuracoes.php', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded',
                        },
                        body: `secao=aparencia&cor_tema=${cor}`
                    })
                    .then(response => response.json())
                    .then(data => {
                        if (data.success) {
                            ...
                            Swal.fire({
                                ...
                                text: 'A cor do tema foi alterada com sucesso.',
                                showConfirmButton: false,
                                timer: 1500,
                                customClass: {
                                    popup: 'border border-theme rounded-lg'
                    ...
                    .catch(error => {
                        Swal.fire({
                            icon: 'error',
                            title: 'Erro',
                            text: error.message || 'Erro ao atualizar o tema',
                            customClass: {
                                popup: 'border border-theme rounded-lg'
                            }
                        });
            ...
            // Adiciona evento para o color picker personalizado
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        Swal.fire({
                            icon: 'success',
                            title: 'Tema Atualizado!',
                            text: 'A cor personalizada foi aplicada com sucesso.',
                            showConfirmButton: false,
                            timer: 1500,
                            customClass: {
                                popup: 'border border-theme rounded-lg'
            ...
            // Log da cor atual do tema
            const corAtual = getComputedStyle(document.documentElement).getPropertyValue('--theme-color').trim();
            console.log('Cor atual do tema (CSS):', corAtual);
            
            // Log do tema salvo no PHP
            console.log('Tema atual (PHP):', '<?php echo $config["cor_tema"] ?? "não definido"; ?>');
        });
    </script>
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    if (secaoBairro && secaoDistancia) {
                        ...
                        console.log('Visibilidade das seções atualizada');
                    } else {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            // Recarrega as listas após salvar
                            carregarBairros();
                            carregarFaixasKm();
                            ...
                            Swal.fire({
                                icon: 'success',
                                title: 'Sucesso!',
                                text: 'Tipo de entrega alterado com sucesso',
                                confirmButtonColor: '#7C3AED',
                                timer: 1500
                            });
                        } else {
                    ...
                    .catch(error => {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Erro',
                            text: error.message || 'Erro ao salvar configuração'
                        });
                    });
                } else {
